...
def parse_af3_results(summary_json_path,confidences_json_path):
    ...
    with open(confidences_json_path,'r') as jfile2:
        ...
        for i, chain1 in enumerate(chains):
            for j, chain2 in enumerate(chains):
                if (i < j):
                    residues_1 = [int(idx) for idx in np.where(residue_chain_ids==chain1)[0]]
                    residues_2 = [int(idx) for idx in np.where(residue_chain_ids==chain2)[0]]
                    sub_pae1 = pae[residues_1[0]:residues_1[-1]+1,residues_2[0]:residues_2[-1]+1]
                    sub_pae2 = pae[residues_2[0]:residues_2[-1]+1,residues_1[0]:residues_1[-1]+1]
                    results[f'avg_pae_interaction_{chain1}_{chain2}'] = 0.5*(np.mean(sub_pae1)+np.mean(sub_pae2))
                    results[f'min_pae_interaction_{chain1}_{chain2}'] = min(np.min(sub_pae1), np.min(sub_pae2))
                    results[f'max_pae_interaction_{chain1}_{chain2}'] = max(np.max(sub_pae1), np.max(sub_pae2))
                    sub_contact_probs = contact_probs[residues_1[0]:residues_1[-1]+1,residues_2[0]:residues_2[-1]+1]
                    results[f'avg_contact_probs_{chain1}_{chain2}'] = np.mean(sub_contact_probs)
                    results[f'min_contact_probs_{chain1}_{chain2}'] = np.min(sub_contact_probs)
                    results[f'max_contact_probs_{chain1}_{chain2}'] = np.max(sub_contact_probs)
                elif (i==j):
                    residues = [int(idx) for idx in np.where(residue_chain_ids==chain1)[0]]
                    sub_pae = pae[residues[0]:residues[-1]+1,residues[0]:residues[-1]+1]
...
def main(args):
    ...
    samples = [0,1,2,3,4] # maybe this is configurable in AF3? see https://github.com/google-deepmind/alphafold3/issues/109
    ...
    for i, (idx,row) in tqdm.tqdm(enumerate(seq_df.iterrows())):
        ...
        for seed in model_seeds:
            for sample in samples:
                ...
                summary_path = f"{path}/seed-{seed}_sample-{sample}/summary_confidences.json"
                confidences_path = f"{path}/seed-{seed}_sample-{sample}/confidences.json"
                sub_results = parse_af3_results(summary_path,confidences_path)
                row.update(sub_results)
                ranking = ranking_df[(ranking_df['seed']==seed)&(ranking_df['sample']==sample)]['ranking_score'].values[0]
                row['af3_ranking'] = ranking
                rows.append(row)

    all_results_df = pd.DataFrame(rows)
    avg_results_df = all_results_df.groupby(by=["original_index"],dropna=False).mean().drop(columns=['af3_seed', 'af3_sample', 'af3_ranking']).reset_index()
